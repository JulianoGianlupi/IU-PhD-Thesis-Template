...
\subsubsection{The Ising Model}\label{sec:abm:cpm-history:ising}
...
\noindent $\delta(a, b)$ is the Kronecker delta function~\cite{} %todo (do I need to cite the Kronecker delta??)}
...
% \textcolor{red}{not the same as the algorithm that evolves the Ising or the Potts model. In those you pick a lattice site and you propose to change it to any allowed value, there's no concept of direction, you can update any site in the lattice, you have detailed balance (anything you do can be undone in the next step). In CPM you can only evaluate against pixel neighbors, there is a copy (movement) direction, and (potentially) a movement length. This has three important consequences: it moves you from an energy formalism to a force formalism, two updates only occur at boundaries, three there's no detailed balance (if a cell disappears it can't come back). These dynamics correspond to biological reality, but they mean that you do not get the equilibrium statistical mechanics that the metropolis algorithm was designed to solve. Another this to note is that, in \ccds, the background (\textit{i.e.}, medium) cannot appear in between cells.}
